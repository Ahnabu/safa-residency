...
interface BookingEmailData extends BaseEmailData {
  ...
  withOutVatAndService: number;
  amount: number;
}

interface StatusUpdateEmailData extends BaseEmailData {
...
interface DeleteBookingEmailData extends BaseEmailData {
  transactionId: string;
  startDate: string;
  endDate: string;
  room: string;
            ...
            <td style="padding: 20px;">
                ...
                <table width="100%" cellpadding="8" cellspacing="0" border="1" style="border-collapse: collapse; margin-bottom: 20px; border-color: #ddd;">
                    ...
                    <tr style="font-size: 12px;">
                        ...
                        <td>${data.endDate}</td>
                        <td>${data.nights}</td>
                        <td>$ ${(data.perNightCost as number).toFixed(2)}</td>
                        <td>$ ${(data.withOutVatAndService as number).toFixed(2)}</td>
                    </tr>
                    <tr style="font-size: 12px;">
                        <td colspan="7" style="text-align: right; padding-right: 15px;">Service Charge</td>
                        <td>$ ${(data.serviceCharge as number).toFixed(2)}</td>
                    </tr>
                    <tr style="font-size: 12px;">
                        <td colspan="7" style="text-align: right; padding-right: 15px;">Govt. Vat</td>
                        <td>$ ${(data.vat as number).toFixed(2)}</td>
                    </tr>
                    <tr style="font-size: 12px; font-weight: bold;">
                        <td colspan="7" style="text-align: right; padding-right: 15px;">Total</td>
                        <td>$ ${(data.amount as number).toFixed(2)}</td>
                    </tr>
                    <tr style="font-size: 12px;">
...
const generateStatusUpdateTemplate = (data: StatusUpdateEmailData) => {
  return `
<!DOCTYPE html>
<html lang="en">
<head>
...
const generateCancellationTemplate = (data: DeleteBookingEmailData) => {
  return `
<!DOCTYPE html>
<html lang="en">
<head>
...
export const EmailHelper = {
  // New booking emails
  sendBookingEmails: async (data: BookingEmailData) => {
    // Send to user
    await sendMailWithTemplate(
      data.email!,
      'Booking Confirmation - Safa Residency',
      generateBookingTemplate(data),
    );
  ...
  // Status update email
  sendStatusUpdateEmail: async (data: StatusUpdateEmailData) => {
    await sendMailWithTemplate(
      data.email!,
      'Booking Status Update - Safa Residency',
      generateStatusUpdateTemplate(data),
    );
  },

  // Cancellation email
  sendCancellationEmail: async (data: DeleteBookingEmailData) => {
    await sendMailWithTemplate(
      data.email!,
      'Booking Cancellation - Safa Residency',
      generateCancellationTemplate(data),
    );
  },
};
